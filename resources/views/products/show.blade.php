...
<div class="container">
    ...
    @php
    $filextension = pathinfo($asset->upload, PATHINFO_EXTENSION);
    @endphp
        ...
        <div class="modal-dialog modal-fullscreen">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <model-viewer alt="{{ $asset->name ?? 'Sem nome' }}" src="{{ asset('storage/' . $asset->upload) }}" ar camera-controls touch-action="pan-y" loading="lazy" class="card-img-top h-100">
                    </model-viewer>
                    @break
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" action="{{ route('products.update', $asset) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group p-2">
                            <select class="form-select @error('tags') is-invalid @enderror" multiple aria-label="tags" id="tags" name="tags[]">

                                @forelse ($tags as $tag)

                                <option value="{{ $tag->id }}" {{ in_array($tag, $asset->tags()->tag_name ?? []) ? 'selected' : '' }}>
                                    {{ $tag ->tag_name}}
                                </option>
...
<div class="p-3 pb-md-4 mx-auto container-fluid">
    ...
    <div class="row row-cols-1 row-cols-md-3 g-4 mb-4 ">
        ...
        <div class="card mx-2 slide-right " style="width: 16rem;">
            ...
            @case($filextension == 'pdf' || $filextension == 'webm')
            <link rel="import" src="{{ asset('storage/' . $asset->upload) }}" title="{{ $asset->name ?? 'Sem nome' }}" class="card-img-top h-100">
            @break
            ...
            @endif
            <div class="card-body">
                <h5 class="">{{ $asset->name ?? 'Sem nome' }}</h5>
